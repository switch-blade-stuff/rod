...
namespace rod
{
	namespace _into_variant
	{
		...
		private:
			...
			template<typename E>
			using error_signs_t = std::conditional_t<value_types_of_t<Snd, E, _detail::all_nothrow_decay_copyable, std::conjunction>::value, completion_signatures<>, completion_signatures<set_error_t(std::exception_ptr)>>;
			template<typename E>
		...
		{
			using back_adaptor = _detail::back_adaptor<into_variant_t>;
			template<typename Snd>
