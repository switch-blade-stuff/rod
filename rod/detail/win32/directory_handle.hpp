...
namespace rod::_win32
{
	...
	inline static result<_dir::directory_handle> reopen_as_deletable(const ntapi &ntapi, const _dir::directory_handle &dir, const fs::file_timeout &to) noexcept
	{
		...
		if (hnd.has_value()) [[likely]]
			return _dir::directory_handle(*hnd, dir.flags());
		else
			return hnd.error();
	}
}
