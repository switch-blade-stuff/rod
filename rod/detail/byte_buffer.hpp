...
namespace rod
{
	...
	[[nodiscard]] constexpr byte_buffer as_bytes(I begin, S end) noexcept(noexcept(std::span(begin, end))) { return std::span(begin, end).as_bytes(); }
	/** Converts a contiguous range of elements to `byte_buffer`. */
	template<std::ranges::contiguous_range R>
	[[nodiscard]] constexpr byte_buffer as_bytes(R &&data) noexcept(noexcept(as_bytes(std::ranges::begin(data), std::ranges::end(data)))) { return as_bytes(std::ranges::begin(data), std::ranges::end(data)); }
	...
	/** Converts a contiguous range of bytes defined by [\a begin, \a end). */
	template<std::contiguous_iterator I, std::sentinel_for<I> S>
	[[nodiscard]] constexpr const_byte_buffer as_const_bytes(I begin, S end) noexcept(noexcept(std::span(begin, end))) { return std::span(begin, end).as_bytes(); }
	/** Converts a contiguous range of elements to `byte_buffer`. */
	template<std::ranges::contiguous_range R>
	[[nodiscard]] constexpr const_byte_buffer as_const_bytes(R &&data) noexcept(noexcept(as_bytes(std::ranges::begin(data), std::ranges::end(data)))) { return as_bytes(std::ranges::begin(data), std::ranges::end(data)); }
