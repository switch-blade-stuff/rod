...
namespace rod
{
	...
	class packed_pair : empty_base<T0>, empty_base<T1>
	{
		using first_base = empty_base<T0>;
		using second_base = empty_base<T1>;
	...
	private:
		...
		static constexpr bool nothrow_constructible = std::conjunction_v<_detail::apply_tuple_t<second_nothrow_constructible, Args1>, _detail::apply_tuple_t<first_nothrow_constructible, Args0>>;
		...
		constexpr packed_pair(std::index_sequence<Is0...>, std::tuple<Args0...> &args0, std::index_sequence<Is1...>, std::tuple<Args1...> &args1)
				: first_base(std::forward<Args0>(std::get<Is0>(args0))...), second_base(std::forward<Args1>(std::get<Is1>(args1))...) {}
	...
	public:
		...
		[[nodiscard]] constexpr first_type &first() & noexcept { return first_base::value(); }
		[[nodiscard]] constexpr first_type &&first() && noexcept { return first_base::value(); }
		[[nodiscard]] constexpr const first_type &first() const & noexcept { return first_base::value(); }
		[[nodiscard]] constexpr const first_type &&first() const && noexcept { return first_base::value(); }

		[[nodiscard]] constexpr second_type &second() & noexcept { return second_base::value(); }
		[[nodiscard]] constexpr second_type &&second() && noexcept { return second_base::value(); }
		[[nodiscard]] constexpr const second_type &second() const & noexcept { return second_base::value(); }
		[[nodiscard]] constexpr const second_type &&second() const && noexcept { return second_base::value(); }
		...
		template<std::integral auto I> requires(I < 2)
		friend constexpr auto &get(const packed_pair<T0, T1> &p) noexcept
		{
			if constexpr (I == 0)
				return p.first();
			else
