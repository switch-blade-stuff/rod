...
namespace rod::io
{
	...
	public:
		...
		template<rod::detail::decays_to<basic_file> T, buffer Dst>
		friend std::size_t tag_invoke(read_t, T &&src, Dst &&dst, std::error_code &err)
		{
		...
		template<rod::detail::decays_to<basic_file> T, buffer Src>
		friend std::size_t tag_invoke(write_t, T &&dst, Src &&src, std::error_code &err)
		{
